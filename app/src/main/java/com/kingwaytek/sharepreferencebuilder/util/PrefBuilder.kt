...
open class PrefBuilder(var aClass: Any, var context: Context) {
    ...
    private fun inject(o: Any, ctx: Context) {
        ...
        for (field in declaredFields) {
            if (field.isAnnotationPresent(PrefName::class.java) && field.getAnnotation(PrefName::class.java) != null) {
                    ...
                    when(field.type) {
                        ...
                        Pref.Object::class.java-> {
                            ...
                            typeParamsOfData?.returnType?.arguments?.singleOrNull()?.let {
                                field.set(o, Pref.Object(ctx, className, prefName, it.type!!.javaType , typeParamsOfData?.returnType.classifier as KClass<*>))
                            }
                        }
                    }
